...
def batch(request):
    ...
    return utils.showSuccess(request, "Batch successful.")
...
def insert(request):
    ...
    res = Collection.objects.filter(table=utils.input(request, "table")).filter(
    ...
    if len(res):
        ...
        return utils.showSuccess(request, "Cancelled.")
    ...
    return utils.showSuccess(request, "Add to my favorite.", referer)
